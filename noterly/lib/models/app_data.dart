...
class AppData {
  ...
  factory AppData.fromJson(Map<String, dynamic> json) => AppData(
        notificationItems: AppDataJsonParser<List<NotificationItem>>(
            'notificationItems', [],
            parser: (value) => (value as List)
                .map((item) => NotificationItem.fromJson(item))
                .toList()).parse(json),
        snoozeDuration: AppDataJsonParser<Duration>(
            'snoozeDuration', const Duration(hours: 1),
            parser: (value) => Duration(seconds: value)).parse(json),
        snoozeToastText:
            AppDataJsonParser<String>('snoozeToastText', 'Notification snoozed')
                .parse(json),
        firstLaunchDialogLastShown:
            AppDataJsonParser<int>('firstLaunchDialogLastShown', -1)
                .parse(json),
      );

  @override
  String toString() =>
      'AppData(notificationItems: $notificationItems, snoozeDuration: $snoozeDuration, snoozeToastText: $snoozeToastText, firstLaunchDialogLastShown: $firstLaunchDialogLastShown)';
}
